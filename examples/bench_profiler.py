...
class ProfilerBenchmark:
    ...
    def list_comprehension_heavy(self) -> List[int]:
        """列表推导式和内置函数调用"""
        return [sum(x * y for y in range(10)) for x in range(100) for _ in range(5)]
...
def compare_overhead():
    ...
    print("Target duration: 10 seconds per test\n")
    ...
    print(
        f"Baseline completed: {baseline_results['iterations']} iterations in {baseline_results['total_time']:.2f}s"
    )

    # 2. 无profiler基准测试，用于测试波动性
    gc.collect()
    time.sleep(1)
    print("Running baseline (no profiler)...")
    baseline2_results = run_benchmark(n, with_profiler=None)
    print(
        f"Baseline completed: {baseline2_results['iterations']} iterations in {baseline2_results['total_time']:.2f}s"
    )

    # 3. 使用probing profiler
    gc.collect()
    time.sleep(1)
    print("\nRunning with simple profiler...")
    profiled_results = run_benchmark(n, with_profiler=True)
    print(
        f"Profiled completed: {profiled_results['iterations']} iterations in {profiled_results['total_time']:.2f}s"
    )

    # 3. 计算开销
    baseline_iter_per_sec = (
        baseline_results["iterations"] / baseline_results["total_time"]
    )
    baseline2_iter_per_sec = (
        baseline2_results["iterations"] / baseline2_results["total_time"]
    )
    profiled_iter_per_sec = (
        profiled_results["iterations"] / profiled_results["total_time"]
    )
    baseline_percent = (
        (baseline_iter_per_sec - baseline2_iter_per_sec) / baseline_iter_per_sec
    ) * 100
    overhead_percent = (
        (baseline_iter_per_sec - profiled_iter_per_sec) / baseline_iter_per_sec
    ) * 100

    print("\n=== Results ===")
    print(f"Baseline performance: {baseline_iter_per_sec:.2f} iterations/second")
    print(
        f"Baseline2 performance: {baseline2_iter_per_sec:.2f} iterations/second, overhead: {baseline_percent:.1f}%"
    )
    print(
        f"Profiled performance: {profiled_iter_per_sec:.2f} iterations/second, overhead: {overhead_percent:.1f}%"
    )
