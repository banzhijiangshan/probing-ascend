...
def start_job_hook():
    ...
    print(
        f"Job started: ID={JOB_ID}, TimestampID={JOB_UNIQUE_ID}, WorldSize={world_size}, PodIP={pod_ip}, TQ_GPU_NUM={tq_gpu_num}"
    )
    data = {
        "jobId": JOB_ID,
...
def record_error_hook(exc_type, exc_value, exc_traceback):
    ...
    print(
        f"Job error: ID={JOB_ID}, TimestampID={JOB_UNIQUE_ID}, Error={str(exc_value)}"
    )
...
print(
    f"Job tracker: Script initialized with JOB_ID: {JOB_ID}, TimestampID: {JOB_UNIQUE_ID}"
)
