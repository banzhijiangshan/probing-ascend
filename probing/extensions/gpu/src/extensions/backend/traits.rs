...
pub struct GpuMemorySample {
    ...
    /// Device / HBM temperature (°C) when the backend provides it (e.g. Ascend DCMI).
    pub temp_c: Option<f32>,
    /// Instantaneous board/chip power (watts) when available (e.g. Ascend DCMI).
    pub power_w: Option<f32>,
    /// Ascend `Aivector Usage Rate(%)` — tensor-engine activity proxy (Minder).
    pub aivector_util_pct: Option<f32>,
    /// Ascend `HBM Bandwidth Usage Rate(%)`.
