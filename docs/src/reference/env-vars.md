...
| `PROBING_TORCH_COLLECTIVE_SYNC` | `0` | When `1`, synchronize the CUDA/NPU device before and after each intercepted `torch.distributed` collective. `python.comm_collective.duration_ms` then includes device completion and rows report `timing_source=device_sync_wall`. The default reports `timing_source=host_api` and measures enqueue/API latency only; do not interpret it as NCCL/HCCL execution time. Enable this only for scoped detailed diagnosis because synchronization can perturb training. |
